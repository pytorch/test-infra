...
if ($config -match '--ephemeral') {
    Add-Content -Path .\.env -Value "ACTIONS_RUNNER_HOOK_JOB_COMPLETED=C:\actions-runner\jobcompleted.ps1"
    Add-Content -Path C:\actions-runner\jobcompleted.ps1 -Value "`$DateTime = (Get-Date).ToUniversalTime()"
    Add-Content -Path C:\actions-runner\jobcompleted.ps1 -Value "`$UnixTimeStamp = [System.Math]::Truncate((Get-Date -Date `$DateTime -UFormat %s))"
    Add-Content -Path C:\actions-runner\jobcompleted.ps1 -Value "aws ec2 create-tags --region $Region --resource $InstanceId --tags `"Key=EphemeralRunnerFinished,Value=`$UnixTimeStamp`""
}
