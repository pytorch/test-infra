...
export async function tryReuseRunner(
  ...
  console.debug(`[tryReuseRunner]: Found ${runners} runners to reuse`);
...
function logReuseFoundOrg(runnerParameters: RunnerInputParameters, metrics: ScaleUpMetrics, runnerLength: number) {
  if (runnerParameters.orgName !== undefined) {
    metrics.runnersReuseFoundOrg(runnerLength, runnerParameters.orgName, runnerParameters.runnerType.runnerTypeName);
  } else if (runnerParameters.repoName !== undefined) {
    metrics.runnersReuseFoundRepo(
      runnerLength,
      getRepo(runnerParameters.repoName),
