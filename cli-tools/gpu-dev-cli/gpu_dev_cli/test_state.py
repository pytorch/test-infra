...
class TestStateManager:
    ...
    def create_reservation(
        ...
        if available_gpus < gpu_count:
            print(
                f"❌ Insufficient GPUs. Requested: {gpu_count}, Available: {available_gpus}"
            )
            return None
    ...
    def list_reservations(
        self, user_filter: Optional[str] = None, status_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def get_cluster_status(self) -> Dict[str, Any]:
        ...
        active_reservations = len(
            [r for r in state["reservations"] if r["status"] == "active"]
        )
