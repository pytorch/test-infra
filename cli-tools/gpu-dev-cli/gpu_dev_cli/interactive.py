...
def check_interactive_support() -> bool:
    ...
    if not sys.stdin.isatty():
        console.print("[yellow]⚠️  Non-interactive terminal detected. Use command-line flags instead.[/yellow]")
        return False
    
    return True


def select_gpu_type_interactive(availability_info: Dict[str, Dict[str, Any]]) -> Optional[str]:
    ...
    for gpu_type, info in availability_info.items():
        ...
        choice_label = f"{status_indicator} {gpu_type.upper()} ({available}/{total} available)"
        if queue_length > 0:
...
def select_gpu_count_interactive(gpu_type: str, max_gpus: int) -> Optional[int]:
    ...
    try:
        answer = questionary.select(
            f"How many {gpu_type.upper()} GPUs?",
            choices=choices,
            style=custom_style
        ).ask()
...
def select_duration_interactive() -> Optional[float]:
    ...
    try:
        answer = questionary.select(
            "How long do you need the reservation?",
            choices=choices,
            style=custom_style
        ).ask()
...
def select_reservation_interactive(reservations: List[Dict[str, Any]], action: str) -> Optional[str]:
    ...
    for reservation in reservations:
        try:
            ...
            console.print(f"[yellow]⚠️  Skipping malformed reservation: {str(row_error)}[/yellow]")
            continue
    ...
    if action == "cancel" and len(choices) > 1:
        choices.append(questionary.Choice(
            title="🗑️  Cancel ALL reservations above",
            value="__ALL__"
        ))
    
    try:
        answer = questionary.select(
            f"Select reservation to {action}:",
            choices=choices,
            style=custom_style
        ).ask()
...
def ask_name_interactive() -> Optional[str]:
    ...
    try:
        answer = questionary.text(
            "Reservation name (optional, press Enter to skip):",
            style=custom_style
        ).ask()
