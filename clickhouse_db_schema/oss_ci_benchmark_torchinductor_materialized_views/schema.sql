...
-- to v3 as I don't want to re-create oss_ci_benchmark_metadata
CREATE TABLE benchmark.oss_ci_benchmark_torchinductor_metadata (
    `device` String,
    `arch` String,
    `model_name` String,
    `metric_name` String,
...
CREATE MATERIALIZED VIEW benchmark.oss_ci_benchmark_torchinductor_metadata_mv TO benchmark.oss_ci_benchmark_torchinductor_metadata AS
SELECT
    IF(
...
-- Below is the SQL query to backfill the view
INSERT INTO
...
WHERE
    ...
    AND timestamp >= toUnixTimestamp(toDateTime('2025-05-01 00:00:00'))
    AND timestamp < toUnixTimestamp(toDateTime('2025-06-20 00:00:00'));

-- To make the query faster by keeping only the metric the dashboard needs
...
SELECT
    ...
    model.origins [ 1 ] AS suite,
    metric.name AS metric_name,
    floor(arrayAvg(metric.benchmark_values), 4) AS value,
    metric.extra_info AS metric_extra_info,
    tupleElement(benchmark, 'dtype') AS benchmark_dtype,
    tupleElement(benchmark, 'mode') AS benchmark_mode,
    benchmark.extra_info AS benchmark_extra_info,
    head_branch AS head_branch,
    head_sha AS head_sha,
    workflow_id AS workflow_id,
    job_id AS job_id,
