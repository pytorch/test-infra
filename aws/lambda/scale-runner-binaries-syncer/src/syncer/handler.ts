...
async function getCachedVersion(
  ...
  try {
    const objectTagging = await s3
      ...
      .promise();
    const versions = objectTagging.TagSet?.filter(
      (t: S3.Tag) => t.Key === versionKey
    );
...
async function getReleaseAsset(
  ...
  const latestPrereleaseIndex = assetsList.data.findIndex(
    (a) => a.prerelease === true
  );
  const latestReleaseIndex = assetsList.data.findIndex(
    (a) => a.prerelease === false
  );
  ...
  const assets = asset.assets?.filter((a) =>
...
async function uploadToS3(
  ...
  await new Promise<void>((resolve, reject) => {
    console.debug(
      "Start downloading %s and uploading to S3.",
      actionRunnerReleaseAsset.name
    );
    request
...
export const handle = async (): Promise<void> => {
  ...
  const fetchPrereleaseBinaries = yn(
    process.env.GITHUB_RUNNER_ALLOW_PRERELEASE_BINARIES,
    { default: false }
  );
  const distributions = [
    {
  ...
  for (const distribution of distributions) {
    ...
    if (
      ...
      currentVersion != actionRunnerReleaseAsset.name
    ) {
