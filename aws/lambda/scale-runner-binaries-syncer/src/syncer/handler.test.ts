...
describe("Synchronize action distribution.", () => {
      ...
      return {
        promise() {
          return Promise.resolve({
            TagSet: [
              { Key: "name", Value: "actions-runner-linux-x64-2.272.0.tar.gz" },
            ],
          });
        },
      };
    ...
    mockS3.getObjectTagging.mockImplementation(() => {
      return {
        promise() {
          return Promise.resolve({
            TagSet: [
              { Key: "name", Value: "actions-runner-linux-x64-2.273.0.tar.gz" },
            ],
          });
        },
      };
  ...
  it("Distribution should update to release.", async () => {
    mockS3.getObjectTagging.mockImplementation(() => {
      return {
        promise() {
          return Promise.resolve({
            TagSet: [
              { Key: "name", Value: "actions-runner-linux-x64-0.tar.gz" },
            ],
          });
        },
      };
    ...
    expect(s3JsonBody["Tagging"]).toEqual(
