...
async function removeRunner(
  ...
  try {
    ...
    console.error(
      `Error scaling down '${ec2runner.instanceId}' [${ec2runner.runnerType}]: ${e}`
    );
  }
}

export async function scaleDown(): Promise<void> {
  const scaleDownConfigs = JSON.parse(
    process.env.SCALE_DOWN_CONFIG as string
  ) as [ScalingDownConfig];

  const enableOrgLevel = false;
  const environment = process.env.ENVIRONMENT as string;
  const minimumRunningTimeInMinutes = process.env
    .MINIMUM_RUNNING_TIME_IN_MINUTES as string;
  ...
  for (const ec2runner of runners) {
    ...
    for (const ghRunner of ghRunners) {
      ...
      if (runnerName === ec2runner.instanceId) {
        ...
        if (ghRunner.busy) {
          console.debug(
            `Runner '${ec2runner.instanceId}' [${ec2runner.runnerType}] is busy, skipping...`
          );
          continue;
        }
        if (idleCounter > 0) {
    ...
    if (orphanEc2Runner) {
      console.info(
        `Runner '${ec2runner.instanceId}' [${ec2runner.runnerType}] is orphan, and will be removed.`
      );
      try {
        await terminateRunner(ec2runner);
      } catch (e) {
        console.error(
          `Orphan runner '${ec2runner.instanceId}' [${ec2runner.runnerType}] cannot be removed: ${e}`
        );
      }
    }
  }
