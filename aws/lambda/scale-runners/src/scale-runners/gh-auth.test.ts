...
describe("Test createGithubAuth", () => {
  test("Creates app client to GitHub public", async () => {
    ...
    // Assert
    expect(result.request.endpoint.DEFAULTS.baseUrl).toBe(
  ...
  test("Creates app client to GitHub ES", async () => {
    ...
    expect(result.request.endpoint.DEFAULTS.mediaType.previews).toStrictEqual([
      "antiope",
    ]);
  });
});
...
describe("Test createGithubAuth", () => {
  ...
  test("Creates auth object for public GitHub", async () => {
    ...
    mockedDecrypt
      .mockResolvedValueOnce(decryptedValue)
  ...
  test("Creates auth object for Enterprise Server", async () => {
    ...
    // Act
    const result = await createGithubAuth(
      installationId,
      authType,
      githubServerUrl
    );
