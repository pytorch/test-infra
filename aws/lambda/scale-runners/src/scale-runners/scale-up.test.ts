...
jest.mock("@octokit/auth-app", () => ({
  createAppAuth: jest
    .fn()
    .mockImplementation(() =>
      jest.fn().mockImplementation(() => ({ token: "Blaat" }))
    ),
}));
const mockOctokit = {
  checks: { get: jest.fn() },
...
beforeEach(() => {
  ...
  mockOctokit.actions.createRegistrationTokenForOrg.mockImplementation(
    () => mockTokenReturnValue
  );
  mockOctokit.actions.createRegistrationTokenForRepo.mockImplementation(
    () => mockTokenReturnValue
  );
  mockOctokit.apps.getOrgInstallation.mockImplementation(
    () => mockInstallationIdReturnValueOrgs
  );
...
describe("scaleUp with GHES", () => {
  ...
  it("ignores non-sqs events", async () => {
    expect.assertions(1);
    expect(scaleUp("aws:s3", TEST_DATA)).rejects.toEqual(
      Error("Cannot handle non-SQS events!")
    );
  });
  ...
  describe("on org level", () => {
    ...
    it("does not create a token when maximum runners has been reached", async () => {
      ...
      await scaleUp("aws:sqs", TEST_DATA);
      expect(
        mockOctokit.actions.createRegistrationTokenForOrg
      ).not.toBeCalled();
    });
