...
export const scaleUp = async (
  ...
  const ghAuth = await createGithubAuth(
    installationId,
    "installation",
    ghesApiUrl
  );
  const githubInstallationClient = await createOctoClient(
    ghAuth.token,
    ghesApiUrl
  );
  ...
  const runnerLabels =
    payload.runnerLabels !== undefined
      ? payload.runnerLabels
      : Array.from(runnerTypes.keys());
  ...
  for (const runnerLabel of runnerLabels) {
    ...
    if (
      ...
      const registrationToken = enableOrgLevel
        ? await githubInstallationClient.actions.createRegistrationTokenForOrg({
            org: payload.repositoryOwner,
          })
      ...
      const runnerGroupArgument =
        runnerGroup !== undefined ? ` --runnergroup ${runnerGroup}` : "";
      ...
      try {
        ...
        console.error(
          `Error spinning up instance of type ${runnerType.runnerTypeName}: ${e}`
        );
      }
    } else {
      console.info(
        "There are available runners, no new runners will be created"
      );
    }
  }
};
...
async function allRunnersBusy(
  ...
  const githubAppClient = await createGitHubClientForRunner(
    org,
    repo,
    enableOrgLevel
  );
  const ghRunners = await listGithubRunners(
    githubAppClient,
    org,
    repo,
    enableOrgLevel
  ...
  const runnersWithLabel = ghRunners.filter(
    (x) =>
      x.labels.some((y) => y.name === runnerType) &&
  ...
  console.info(
    `Found matching GitHub runners [${runnerType}], ${busyCount}/${runnersWithLabel.length} are busy`
  );
...
async function GetRunnerTypes(
  ...
  const result: Map<string, RunnerType> = new Map<string, RunnerType>();
