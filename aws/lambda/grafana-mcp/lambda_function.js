...
async function downloadSessionFromS3(userUuid, tempDir) {
  ...
  try {
    ...
    if (error.name === 'NoSuchKey') {
      console.log(`No existing session found for user ${userUuid}, starting fresh`);
      return false;
    }
...
exports.handler = awslambda.streamifyResponse(async (event, responseStream, context) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    ...
    responseStream.write('{"error":"Missing or invalid authorization header"}\n');
    responseStream.end();
    return;
  }
  ...
  if (!query || typeof query !== 'string') {
    ...
    responseStream.write('{"error":"Query parameter is required and must be a string"}\n');
    responseStream.end();
    return;
  }
  ...
  if (!userUuid) {
    ...
    responseStream.write('{"error":"userUuid parameter is required for session management"}\n');
    responseStream.end();
    return;
  }

  console.log(`Processing query (${query.length} chars) for user ${userUuid}`);
  ...
  try {
    ...
    try {
      ...
      const grafanaUrl = process.env.GRAFANA_MCP_URL || 'http://grafana-mcp.grafana-mcp-lambda.local:8000';
      const clickhouseUrl = process.env.CLICKHOUSE_MCP_URL || 'http://clickhouse-mcp.grafana-mcp-lambda.local:8001';
    ...
    responseStream.write(`{"status":"starting","tempDir":"${tempDir}","userUuid":"${userUuid}"}\n`);

    // Launch Claude process with claude command directly (exactly like torchci)
    claudeProcess = spawn('claude', [
      '-p',
      'Use TodoRead/TodoWrite to create a plan first (find tables, understand tables, create query, optimize query for Grafana, make Grafana dashboard). ' + query,
    ...
    claudeProcess.on('error', (error) => {
      ...
      if (!isResponseEnded) {
        responseStream.write(`{"error":"${error.message.replace(/"/g, '\\"')}"}\n`);
      }
      safeEndResponse();
    });
    ...
    claudeProcess.on('close', (code) => {
      ...
      uploadSessionToS3(userUuid, tempDir).then(() => {
