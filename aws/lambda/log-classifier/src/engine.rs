...
fn get_line_number_chunks(
    ...
    for line_number in line_numbers {
        if chunk.is_empty() {
            chunk.push(line_number);
        } else if line_number - chunk.first().unwrap() < effective_max_chunk_size {
            chunk.push(line_number);
        } else {
            let start = max(chunk.first().unwrap() - min_chunk_size / 2, 1);
            let end = min(chunk.last().unwrap() + min_chunk_size / 2, log.lines.len());
            merged_line_numbers.push((start, end));
            chunk.clear();
            chunk.push(line_number);
        }
    }
...
mod test {
    ...
    fn test_get_line_number_chunks_smoke_test() {
        ...
        assert_eq_unordered!(chunks, [(1, 1), (2, 2)].into());
    }
    ...
    fn test_get_line_number_chunks_multiple_chunks() {
        ...
        assert_eq_unordered!(chunks, [(1, 2), (4, 4)].into());
    }
    ...
    fn test_get_line_number_chunks_overlap() {
        ...
        assert_eq_unordered!(chunks, [(1, 3), (2, 5), (4, 6)].into());
    }
