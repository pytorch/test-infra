...
/// the chunks by `min_chunk_size / 2` on both sides, up to the limits of the log which could
/// lead to overlapping chunks.
///
/// # Panics
///
/// This function will panic if:
/// * `min_chunk_size` is greater than or equal to `max_chunk_size`.
...
fn get_line_number_chunks(
    ...
    // throw user error if min_chunk_size is greater than max_chunk_size
    if min_chunk_size >= max_chunk_size {
        panic!("min_chunk_size cannot be greater or equal to max_chunk_size");
    }
    ...
    // as we add min_chunk_size / 2 to both sides
    let effective_max_chunk_size = max_chunk_size - min_chunk_size;
    ...
    for line_number in line_numbers {
        if chunk_start == 0 {
            ...
            chunk_start = max(chunk_start - min_chunk_size / 2, 1);
            chunk_end = min(
                chunk_end + min_chunk_size / 2 + min_chunk_size % 2,
                log.lines.len(),
            );

            if chunk_end - chunk_start < min_chunk_size {
                chunk_end = min(chunk_start + min_chunk_size, log.lines.len());
                chunk_start = max(chunk_end - min_chunk_size, 1);
            }
    ...
    merged_line_numbers.push((
        max(chunk_start - min_chunk_size / 2, 1),
        min(
            chunk_end + min_chunk_size / 2 + min_chunk_size % 2,
            log.lines.len(),
        ),
...
pub fn get_snippets(
    ...
    let chunks = get_line_number_chunks(log, line_numbers, min_chunk_size, max_chunk_size);
    chunks
        .iter()
        .map(|(start, end)| {
...
mod test {
    ...
    fn test_get_line_number_chunks_smoke_test() {
        ...
        let chunks = get_line_number_chunks(&log, line_numbers, 1, 2);
    ...
    fn test_get_line_number_chunks_multiple_chunks() {
        ...
        assert_eq_unordered!(chunks, [(1, 3), (4, 5)].into());
    }
    ...
    fn test_get_line_number_chunks_overlap() {
        ...
        let chunks = get_line_number_chunks(&log, line_numbers, 2, 4);
        assert_eq_unordered!(chunks, [(1, 4), (3, 6)].into());
    }
    ...
    fn test_get_snippets() {
        ...
        let snippets = get_snippets(&log, line_numbers, 2, 4);
        assert_eq_unordered!(
            snippets,
            [
                "test foo\ntest bar\ntest baz\ntest foo2\n",
    ...
    fn test_get_snippets_on_log() {
        ...
        // Call the function
        let result = get_snippets(&log, error_line, num_lines, num_lines + 1);
        // length is 1
