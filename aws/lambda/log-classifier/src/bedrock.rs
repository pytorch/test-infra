...
fn validate_output_in_log(ai_output: &str, log: &Log) -> Option<String> {
    ...
    let extracted_error_line = ai_output
        .find(start_tag)
        .and_then(|start_index| {
            ai_output[start_index + start_tag.len()..]
                .find(end_tag)
                .map(|end_index| {
                    let content = &ai_output[start_index + start_tag.len()..start_index + start_tag.len() + end_index];
...
mod test {
    ...
    // use crate::bedrock::create_log_snippet;
    // use crate::bedrock::make_query;
    use crate::log::Log;
    use std::fs;
    ...
    fn test_validate_output_in_log() {
        ...
        assert_eq!(validation_result, Some("##[error]Process completed with exit code 1.".to_string()));
    }
