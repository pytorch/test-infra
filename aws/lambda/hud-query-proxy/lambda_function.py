...
async def run_query(query: str, params: Any = None) -> List[Dict[str, Any]]:
    eprint(f"Executing '{query}' with params: {params}")

    my_con = await aiomysql.connect(
        host=os.environ["db_host"],
        port=socket.getservbyname("mysql"),
...
async def gha_jobs_for_shas(shas: List[str]) -> List[Any]:
...
async def circleci_jobs_for_shas(shas: List[str]) -> List[Any]:
    placeholders = ["%s"] * len(shas)
    query = f"""
        select context,
            sha,
...
def deduplicate_jobs(commit: Dict[str, Any]) -> None:
    deduplicated_jobs: Dict[str, Any] = {}
