...
describe("handler", () => {
  ...
  it("handles check_run events", async () => {
    const resp = await handle(
      {
        "X-Hub-Signature": "sha1=4a82d2f60346e16dab3546eb3b56d8dde4d5b659",
  ...
  it("does not handle check_run events with actions other than created", async () => {
    ...
    const resp = await handle(
      {
        "X-Hub-Signature": "sha1=891749859807857017f7ee56a429e8fcead6f3e1",
        "X-GitHub-Event": "push",
      },
  ...
  it("does not handle check_run events with status other than queued", async () => {
    const event = {
      ...check_run_event,
      check_run: { id: 1234, status: "completed" },
    };
    const resp = await handle(
      {
        "X-Hub-Signature": "sha1=73dfae4aa56de5b038af8921b40d7a412ce7ca19",
        "X-GitHub-Event": "push",
      },
      JSON.stringify(event)
    );
