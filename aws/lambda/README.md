...
Go to [pytorch-gha-infra](https://github.com/metainternal/pytorch-gha-infra)

### Setup Permission and Deployment for the lambda function
   - Add your lambda function config (such as create role, func-name, and permissions), similar to [PR: set up Aws Resources for queue time histogram lambda ](https://github.com/metainternal/pytorch-gha-infra/pull/647)
   ...
   - Update the release-tag and add your zip file name in [runners/common/Terrafile](https://github.com/metainternal/pytorch-gha-infra/blob/5fde9cdadaad584de3140488adba8eb9c9fe6722/runners/common/Terrafile)
   ...
   - If you need ClickHouse account permission, you need ask pytorch dev infra teammate to create a ClickHouse role for your lambda.
       - Add the ClickHouse role secret to the repo secret,  `bunnylol oss metainternal/pytorch-gha-infra` and update it in settings-> secrets.

### Deploy the lambda
Once the pr is submitted, go to [Runners Do Terraform Release (apply)](https://github.com/metainternal/pytorch-gha-infra/actions/workflows/runners-on-dispatch-release.yml), and click Run workflow.
