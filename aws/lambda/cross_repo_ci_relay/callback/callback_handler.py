    ...
    *,
    ...
    try:
        ...
        cr_id = gh_helper.create_check_run(
            ...
            # Store the downstream job_id so a check-run rerequest can re-run
            # exactly this job (not the whole workflow run). The action
            # guarantees job_id is present, so no fallback here.
