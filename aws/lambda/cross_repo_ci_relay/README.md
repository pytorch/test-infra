...
- **State machine**: Relay maintains a **unified state machine** in Redis to validate callback lifecycles, compute timing metrics, and support per-job tracking:
  ...
  - **Timing metrics**: `execution_time = in_progress_timestamp → completed_timestamp`, per job. `queue_time = dispatch_timestamp → first_job_in_progress_timestamp` is workflow-level, not per-job: a run's jobs commonly wait on one another (e.g. tests waiting on a build job) but share one dispatch timestamp, so only the first job in the run to report `in_progress` computes and reports `queue_time`; later jobs in the same run report `queue_time: null` so each run contributes exactly one sample. Timestamps extracted from state records.
