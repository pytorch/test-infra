...
use aws_sdk_ec2::{Client, types::{LaunchTemplate, LaunchTemplateVersion}};
use crate::DiffError;
...
impl Ec2Client for AwsEc2Client {
    async fn get_launch_template_by_name(&self, name: &str) -> Result<LaunchTemplate, DiffError> {
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| DiffError::AwsError(format!("Failed to describe launch template: {}", e)))?;
        
        response.launch_templates
            .unwrap_or_default()
            .into_iter()
            .next()
    ...
    async fn list_launch_template_versions(&self, template_id: &str) -> Result<Vec<LaunchTemplateVersion>, DiffError> {
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| DiffError::AwsError(format!("Failed to list launch template versions: {}", e)))?;
    ...
    async fn get_launch_template_version(&self, template_id: &str, version: &str) -> Result<LaunchTemplateVersion, DiffError> {
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| DiffError::AwsError(format!("Failed to get launch template version: {}", e)))?;
        
        response.launch_template_versions
            .unwrap_or_default()
            .into_iter()
            .next()
            .ok_or_else(|| DiffError::NotFound(format!("Launch template version not found: {}", version)))
    }
}