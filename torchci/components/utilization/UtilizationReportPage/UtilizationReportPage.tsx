...
const WorkflowUtilization = () => {
  ...
  const { group_by, granularity='day', start_time = dayjs.utc().format('YYYY-MM-DD'), end_time=dayjs.utc().format('YYYY-MM-DD') } = router.query;
  const params:any= {
  ...
  const metricsKeys = Array.from(
    new Set(
      data.list.flatMap((job) => Object.keys(job?.metrics || {}))
    )
  );
...
function useUtilReports(params:any): {
  ...
  const url = `/api/list_util_reports/${params.group_by}?${queryParams.toString()}`;
