...
import { CommitMeta } from "lib/benchmark/store/benchmark_regression_store";
import { useEffect, useState } from "react";
import { DISPLAY_NAMES_TO_ARCH_NAMES, DISPLAY_NAMES_TO_DEVICE_NAMES } from "../compilers/common";
import { UMDenseCommitDropdown } from "components/uiModules/UMDenseRenderers";
...
export function CommitChoiceSection({ benchmarkId }: { benchmarkId?: string }) {
  ...
  const [leftList, setLeftList]   = useState<CommitMeta[]>([]);
  const [rightList, setRightList] = useState<CommitMeta[]>([]);
  ...
  // Helper
  const inList = (list: CommitMeta[], workflow_id?: string | null) =>
    !!workflow_id && list.some((c) => c.workflow_id === workflow_id);

  // Sync lists & auto-picks when data / selection changes
  useEffect(() => {
    const L: CommitMeta[] | undefined = data?.left ?? data?.data ?? [];
    const R: CommitMeta[] | undefined = data?.right ?? data?.data ?? [];
    if (!L || !R) return;
