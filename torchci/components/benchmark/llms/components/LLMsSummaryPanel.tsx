...
const processJobLevelFailureRows = (
  ...
  Object.keys(dataGroupedByModel).forEach((key: string) => {
    ...
    jobLevelFailureKeys.forEach((failureKey: string) => {
      // get failure info from the job-level-failure row
      const hasLFailure =
        "l" in dataGroupedByModel[failureKey]["FAILURE_REPORT"];
      const hasRFailure =
        "r" in dataGroupedByModel[failureKey]["FAILURE_REPORT"];
