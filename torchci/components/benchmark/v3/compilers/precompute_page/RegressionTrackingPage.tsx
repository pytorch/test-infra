...
import { useBenchmarkCompilerData } from "lib/benchmark/api_helper/compilers/type";
  ...
  return (
    <DashboardStoreProvider key={'compiler_precompute'} initial={initial}>
      <div style={{ display: "flex" }}>
        ...
        <main style={{ flex: 1 }}>
          <DataRender benchmarkId={'compiler_precompute'} />
        </main>
      </div>
    </DashboardStoreProvider>
  );
...
function DataRender({ benchmarkId }: { benchmarkId?: string }) {
  ...
  const { data, isLoading, error } = useBenchmarkCompilerData(
    "compiler_precompute",
    queryParams
  );
  if (isLoading) {
