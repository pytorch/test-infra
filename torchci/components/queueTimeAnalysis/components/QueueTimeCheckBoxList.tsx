...
export default function QueueTimeCheckBoxList(
{
    ...
    const [category, setCategory] = useState<string>(inputCategory? inputCategory: "workflow_name");
    const timeParams ={
        startTime: startDate.utc().format("YYYY-MM-DDTHH:mm:ss"),
        endTime: endDate.utc().format("YYYY-MM-DDTHH:mm:ss"),
    }

    const { data, isLoading } = useClickHouseAPIImmutable("queue_time_analysis/queue_time_search_items", timeParams);
    ...
    useEffect(() => {
        ...
        newSelectedItem = items.reduce((acc: { [x: string]: boolean; }, item: string) => {
            acc[item] = inputSet.has(item);
            return acc;
        }, {});

        const selected = Object.keys(newSelectedItem).filter((key) => newSelectedItem[key]);
