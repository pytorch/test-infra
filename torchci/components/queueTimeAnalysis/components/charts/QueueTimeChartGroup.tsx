...
export default function QueueTimeChartGroup({props}:{props: any}) {
  ...
  return (
    <>
     <ToggleIconPicker toggleList={chartToggleList} type ={chartType} setType={(val: any) => {
                              setChartType(val)
                              }}
                      />
    {error && error.status ===431 && <Alert severity="error"> Search Request failed with code: 431 Request header fields too large, please select less items in search</Alert>}
    <FlexNoWrap>
      <QueueTimeChartUI data={data} granularity={props.granularity} chartType={chartType} chartGroup="test1"/>
      {data && chartType === "heatmap" && <QueueTimeChartUI data={data} granularity={props.granularity} chartType={'histogram_bar_vertical'} chartGroup="test1" width="600px"/>}
    </FlexNoWrap>
    </>
  );
}
