...
export default function QueueTimeSearchBar({
  ...
  updateSearch: React.Dispatch<any>;
      ...
      useEffect(() => {
        ...
        const newprops = {
          dateRange: rQuery.dateRange? parseInt(rQuery.dateRange as string): rQuery.startDate || rQuery.endDate? -1: 7,
          startDate: rQuery.startDate? toHalfHourDayJs(rQuery.startDate as string): rQuery.dateRange? toHalfHourDayJs(dayjs().format()).subtract(parseInt(rQuery.dateRange as string), "day"): toHalfHourDayJs(dayjs().format()).subtract(7, "day"),
          endDate: rQuery.endDate ? toHalfHourDayJs(rQuery.endDate as string) : toHalfHourDayJs(dayjs().format()),
          granularity: rQuery.granularity as string|| "hour",
          chartType: rQuery.chartType as string || "bar",
          repos: rQuery.repos? splitString(rQuery.repos as string): ["pytorch/pytorch"],
        ...
        return (
            ...
            <HorizontalDiv>
                  ...
                  />
                <TimeGranuityPicker enableHalfHour={true} granularity={props.granularity} setGranularity={(val: any) => {
                    dispatch({ type: "UPDATE_FIELD", field: "granularity", value: val });
                }} />
            </HorizontalDiv>
            <div>
