WITH
        ...
        SELECT
            ...
            SUM(
                CASE
                    ...
                    ELSE 0
                END
            ) * 100 / (COUNT_IF(conclusion != 'cancelled') + 1) as success_rate,
            -- plus one is to handle divide by zero errors
            SUM(
                CASE
            ...
            SUM(
                CASE
                    ...
                    ELSE 0
                END
            ) * 100 / COUNT(*) as cancelled_rate,
            -- cancelled rate is calculated over all jobs
            SUM(
                CASE
                    WHEN conclusion = 'success' THEN 1
                    ELSE 0
    ...
    comparison_stats AS (
        SELECT
            ...
            SUM(m.group_size) as meta,
            SUM(lf.group_size) as lf,
            CAST(SUM(m.group_size) as FLOAT) / SUM(lf.group_size + m.group_size) * 100 as percentage_meta,
            CAST(SUM(lf.group_size) as FLOAT) / SUM(lf.group_size + m.group_size) * 100 as percentage_lf
        FROM
            success_stats lf
...
from
    comparison_stats
ORDER BY
    bucket DESC -- ORDER by bucket desc, job_name desc, success_rate_delta, workflow_name
