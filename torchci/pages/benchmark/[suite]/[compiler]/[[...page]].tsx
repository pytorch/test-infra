...
import { GridValueFormatterParams, GridRenderCellParams, GridCellParams } from "@mui/x-data-grid";
    ...
    <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
      <Typography fontSize={"1rem"} fontStyle={"italic"}>
        *This report was last generated by{" "}
        <a href={`${HUD_PREFIX}/${latestSha}`}>{latestSha.substring(0, 7)}</a> on {dayjs(latestDate).format("YYYY/MM/DD")}.
      </Typography>
    </Stack>
  );
}

function selectRecordsByIndex(
  data: any,
  bucketIndex: number,
) {
  const timeFieldName = "granularity_bucket";
  const buckets = new Set<string>(data.map((record: any) => record[timeFieldName]));

  if (bucketIndex >= buckets.size) {
    return data;
  }

  const selectBucket = Array.from(buckets).sort((a: string, b: string) => (a > b ? -1 : 1))[bucketIndex];
          ...
          columns={[
            {
              ...
              cellClassName: (params: GridCellParams<string>) => {
                return modelName !== undefined && params.value === modelName ? styles.selectedRow : styles.name;
              },
            ...
            },
            {
              ...
              cellClassName: (params: GridCellParams<string>) => {
                ...
                return Number(v) < COMPILATION_lATENCY_THRESHOLD_IN_SECONDS && Number(v) > 0 ? "" : styles.warning;
              },
            },
            {
...
function GraphPanel({
  ...
  const chartData = records.filter((record: any) => record["name"] == modelName);
...
function Report({
  ...
  return (
    <div>
      ...
      <GraphPanel suite={suite} compiler={compiler} modelName={modelName} records={data} startTime={startTime} stopTime={stopTime} granularity={granularity}/>
      <ModelsPanel suite={suite} compiler={compiler} modelName={modelName} records={selectRecords}/>
    </div>
  );
}
