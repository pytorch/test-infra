...
function ValidationRedPanel({ params, channel, query_type }: { params: RocksetParam[], channel: string, query_type: string }) {
  ...
  const options: EChartsOption = {
    title: {
      text: channel.charAt(0).toUpperCase()+channel.slice(1)+
      " "+
      query_type.charAt(0).toUpperCase()+query_type.slice(1)+
      " workflows failures, by day"
    },
