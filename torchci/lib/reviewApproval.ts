...
//
// For example, both the merge command and the PR Status line in the pytorchbot
// comment should agree on the PR's approval status and will use this module as
// the source of truth.
//
// Pure: takes an already-fetched review list that the caller is responsible for.
 ...
 * the unrecognised-review-state notice
 */
  ...
  var latest_reviews: { [user: string]: string } = [...reviews]
    ...
    .reduce(
      (
        ...
        // Casing is weird here. The typescript definition says state will be lower case, yet github
        // returns upper case. We can't trust that to remain that way, so always convert the state
