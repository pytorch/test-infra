...
import { NotFoundComponent, resolveComponent } from "components/benchmark/v3/configs/configRegistration";
import { CompilerPrecomputeBenchmarkUIConfig, COMPILTER_PRECOMPUTE_BENCHMARK_ID } from "components/benchmark/v3/configs/teams/compilers/config";
import { defaultDashboardBenchmarkUIConfig } from "components/benchmark/v3/configs/teams/default/default_dashboard_config";
import { DataBinding, DataBindingConfig } from "components/benchmark/v3/configs/utils/dataBindingRegistration";
...
export class BenchmarkUIConfigHandler {
  ...
  getDataRenderComponent = (): React.ComponentType<any> => {
    ...
    if(!dr) {
      throw new Error(`No data render config found for ${this._benchmarkId}, this is internal error, please report `);
    }
    switch (dr.type) {
      case "fanout":
...
interface State {
  ...
  initTempConfig: (id: string, type?: string, params?: Partial<BenchmarkUIConfig>) => BenchmarkUIConfig;
  ensureConfig: (id: string, type?: string, params?: Partial<BenchmarkUIConfig>) => BenchmarkUIConfigHandler;
