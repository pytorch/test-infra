    ...
    for (const level of ["L1", "L2", "L3", "L4"] as AllowlistLevel[]) {
      if (level === "L3") {
        ...
        if (typeof l3Raw !== "object" || Array.isArray(l3Raw)) {
          throw new Error(
            `Invalid allowlist: L3 must be a device mapping, got ${Array.isArray(l3Raw) ? "list" : typeof l3Raw}`
          );
        }
        ...
        for (const [device, reposRaw] of Object.entries(devices)) {
          ...
          if (!trimmedDevice) {
            throw new Error("Invalid allowlist: L3 device name must not be empty");
          }
          ...
          for (const [repoRaw, oncallsRaw] of Object.entries(repoMap)) {
            const repo = String(repoRaw).trim().replace(/^\/|\/$/g, "");
...
export async function fetchCrcrAllowlist(
  ...
  if (cachedAllowlist && Date.now() - cachedAllowlist.ts < CACHE_TTL_MS) {
  ...
  const yamlStr = Buffer.from(response.data.content, "base64").toString("utf-8");
