// Shared PR-level false-positive verification used by both autorevert API
// endpoints (`pages/api/autorevert/metrics.ts` and
// `pages/api/autorevert/false-positives.ts`).
//
// Background: PyTorch lands via `pytorchmergebot` cherry-pick, so GitHub's
// `merged_at` is unreliable; the `Merged` label is the truth signal that a PR
// shipped. But both `Merged` and `Reverted` labels are STICKY — added by
// mergebot on each cycle, never removed even after a subsequent revert. A PR
// that goes merge → revert → abandon-without-reland still carries the `Merged`
// label, looks "closed-and-was-merged-with-no-post-revert-commits", and gets
// mis-classified as a confirmed FP.
//
...
export function countCommitsAfter(
  ...
  return commits.filter((c) => {
    const ts = new Date(
      c.commit.committer?.date || c.commit.author?.date || 0
    );
    return ts > cutoff;
  }).length;
}
