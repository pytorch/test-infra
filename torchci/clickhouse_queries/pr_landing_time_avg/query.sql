...
WITH 
    ...
    pr_landing_times AS (
        SELECT 
            ms.pr_number,
            DATE_DIFF('minute', ms.first_merge_attempt, mp.merge_time) / 60.0 AS landing_time_hours
        FROM merge_started ms
        INNER JOIN merged_prs mp ON ms.pr_number = mp.pr_number
        WHERE 
            mp.merge_time > ms.first_merge_attempt  -- Ensure merge happened after merge attempt
...
SELECT 
    ROUND(AVG(landing_time_hours), 2) AS avg_hours
