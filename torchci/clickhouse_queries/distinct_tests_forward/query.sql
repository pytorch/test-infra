...
multiIf(
    ...
    {sort_field: String} = 'averageDuration',
    {cursor_average_duration_ms: UInt64},
    {sort_field: String} = 'lastRun',
    toUInt64({cursor_last_run_ns: String}),
    toUInt64(0)
    ...
    OR (
        ...
        AND (
            (
                ...
                AND (
                    tuple(sort_text, sort_number)
                    > tuple(cursor_sort_text, cursor_sort_number)
                    OR (
                        tuple(sort_text, sort_number)
                        = tuple(cursor_sort_text, cursor_sort_number)
...
ORDER BY
    ...
    if({sort_ascending: UInt8} = 0, sort_number, toUInt64(0)) DESC,
    name ASC,
    classname ASC,
    file ASC
LIMIT {limit: UInt32}
