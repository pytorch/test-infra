...
describe("classifyFp decision tree", () => {
  ...
  it("terminal close by non-mergebot -> legit_revert (the fix; PR #182078 shape)", () => {
    // The canonical case: PR went through merge->revert cycles, mergebot
    // closed+reopened on each cycle, then the AUTHOR closed manually after
    // the last revert. Sticky `Merged` label is present but the author's
    // close is the authoritative signal that the PR was abandoned.
    const result = classifyFp({
