...
def main(filename, region='us-east-1', dryrun=False):
    ...
    print(f"Found {len(instance_ids)} instances in {filename} - region {region} - dryrun {dryrun}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filename', required=True, help='File containing instance IDs')
    parser.add_argument('--dryrun', action='store_true', help='Dry run mode (do not actually terminate instances)')
    parser.add_argument('--region', help='region to query ec2 instances from', default='us-east-1')
