...
def get_arguments():
    parser = argparse.ArgumentParser(
        description=f"Consolidate {SCALE_CFG_PATH} files from multiple repos."
    )
    parser.add_argument(
        "-r",
        "--repos",
    ...
    parser.add_argument(
        "-u", "--user", type=str, required=False, default=None, help="github user"
    )
    parser.add_argument(
        "-t", "--token", type=str, required=False, default=None, help="github password"
    )
...
def get_repo_scale_cfg(org, repo, user, token):
    for branch in (
        ...
        url = (
            f"https://raw.githubusercontent.com/{org}/{repo}/{branch}/{SCALE_CFG_PATH}"
        )
        wait_time = 10
        while True:
            if user and token:
                response = requests.get(
                    url, auth=requests.auth.HTTPBasicAuth(user, token)
                )
            else:
...
def check_being_used(org, repo, name, user, token):
    ...
    query = {"q": f"{name} in:file path:.github repo:{org}/{repo}"}
    wait_time = 10
    while True:
        if user and token:
            response = requests.get(
                url, params=query, auth=requests.auth.HTTPBasicAuth(user, token)
            )
        else:
        ...
        for k, v in scale_cfg["runner_types"].items():
            ...
            for sleep_time in sleep_times:
                if check_being_used(args.org, repo, k, args.user, args.token):
                    ...
                    break
                print(
                    f"WARNING! Found a instance_type ({k}) that seems to not be used in repo {args.org}/{repo} (will repeat, as it can be rate limiting)"
                )
                time.sleep(sleep_time)

            if not found_in_repo:
                print(
                    f"WARNING! Found a instance_type ({k}) that seems to not be used in repo {args.org}/{repo} REALLY IGNORED!",
                    file=sys.stderr,
                )
                continue
            ...
            try:
                ...
                if v["instance_type"] in type_warning:
                    if k in type_warning[v["instance_type"]]:
                        ...
                        for runn_type, runn_cfg in type_warning[
                            v["instance_type"]
                        ].items():
                ...
                if k in consolidated["runner_types"]:
                    if (
                        consolidated["runner_types"][k]["instance_type"]
                        != v["instance_type"]
                    ):
                        show_error(
                            "Runners with same name MUST have same instance_type",
                            k,
                            v,
                            repo,
                            type_warning,
                    ...
                    if (
                        consolidated["runner_types"][k]["is_ephemeral"]
                        != v["is_ephemeral"]
                    ):
                        show_error(
                            "Runners with same name MUST have same is_ephemeral flag",
                            k,
                            v,
                            repo,
                            type_warning,
                    ...
                    if consolidated["runner_types"][k]["os"] != v["os"]:
                        show_error(
                            "Runners with same name MUST have same os",
                            k,
                            v,
                            repo,
                            type_warning,
