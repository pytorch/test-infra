...
def get_runners_names(prefix: str, scale_config: str) -> list[str]:
    ...
    runners_w_prefix = set(runner_name.replace(prefix, "") for runner_name in config["runner_types"].keys() if runner_name.startswith(prefix))
    runners_wo_prefix = [runner_name for runner_name in config["runner_types"].keys() if not runner_name.startswith(prefix) and runner_name in runners_w_prefix]
    runners_wo_prefix.sort()

    runners = [runners_wo_prefix[0], ]
...
def commit_push_open_pr(repo_name: str, temp_folder: str, branch_name: str, comment: str) -> None:
    subprocess.run(["git", "add", "-A"], cwd=f"{temp_folder}/{repo_name}", )
    subprocess.run(["git", "commit", "-m", comment], cwd=f"{temp_folder}/{repo_name}", )
    subprocess.run(["git", "push", "origin", branch_name], cwd=f"{temp_folder}/{repo_name}", )
    subprocess.run(
        [
            "gh", "pr", "create",
            "--repo", f"pytorch/{repo_name}",
            "--base", "main",
            "--head", branch_name,
            "--title", comment,
            "--body", f"testing new runners",
        ],
...
def open_branch(repo, repo_name: str, temp_folder: str, branch_name: str) -> None:
    subprocess.run(["git", "clone", repo, f"{temp_folder}/{repo_name}", ])
    subprocess.run(["git", "branch", branch_name, ], cwd=f"{temp_folder}/{repo_name}")
    subprocess.run(["git", "checkout", branch_name, ], cwd=f"{temp_folder}/{repo_name}")
...
def main() -> None:
    ...
    subprocess.run(["rm", "-rf", opts.temp_folder, ])
    subprocess.run(["mkdir", "-p", opts.temp_folder, ])
    try:
        for repo in REPOS:
            repo_name = repo.split('/')[-1]
            open_branch(repo, repo_name, opts.temp_folder, branch_name)
            find_replace(f"{opts.temp_folder}/{repo_name}", runners, opts.prefix, "*")
            commit_push_open_pr(repo_name, opts.temp_folder, branch_name, f"Replace runners prefix {opts.prefix}")
    finally:
        pass
        subprocess.run(["rm", "-rf", opts.temp_folder, ])
