...
valid_json_invalid_threshold_alert_schema = json.dumps([{
    ...
    "Flags": ["Flag1", "Flag2"],
    "branch": "main",
    "MeasurementName": 1, # should be string
    "ThresholdDescription": "test",
}])

valid_json_valid_threshold_alert_schema = json.dumps([{
    "AlertType": "ThresholdAlert",
    "AlertObject": "Bar",
    "OncallTeams": ["Team1", "Team2"],
    "OncallIndividuals": ["Individual1", "Individual2"],
...
class AlertValidationTest(TestCase):
    ...
    def test_validate_threshold_alert_schema(self):
        # Test whether valid json that conforms to the schema is correctly validated
        assert validate_alerts.validate_schema(valid_json_valid_threshold_alert_schema) is None

        # Test whether valid json that does not conform to the schema raises an error
        with self.assertRaises(jsonschema.exceptions.ValidationError):
            validate_alerts.validate_schema(valid_json_invalid_threshold_alert_schema)
