...
def _check_by_position(
    ...
    before_param_names = [
        param.name for param in before.parameters if param.position is not None
    ]
    after_param_names = [
        param.name for param in after.parameters if param.position is not None
    ]
    ...
    matcher = difflib.SequenceMatcher(
        a=[param.name for param in before_params],
        b=[param.name for param in after_params],
    )
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        if tag == 'insert':
            ...
            continue
        if tag == 'delete':
            before_param = before_params[i1]
            yield Violation(
                func,
                f'{before_param.name} was removed',
                line=after.line,
            )
