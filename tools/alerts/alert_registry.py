...
def generate_failed_job_issue(
    ...
    issue[
        "title"
    ] = f"[{alerts[0]['repo']}] [{alerts[0]['AlertType']}] [Branch: {alerts[0]['branch']}] [TEST ALERT PAY NO ATTENTION TO THIS] There are {len([alert for alert in alerts if not alert['closed']])} failing jobs"
