...
def validate_schema(json_string):
    ...
    for alert_type, alert_schema_add_on in EXTRA_ALERT_PROPERTIES.items():
        all_alerts_schemas[alert_type]["items"]["properties"].update(
            alert_schema_add_on
        )
    ...
    for alert in json_object:
        jsonschema.validate(
            instance=[alert], schema=all_alerts_schemas[alert["AlertType"]]
        )


def main():
    parser = argparse.ArgumentParser(
        description="Validate json string containing alerts"
    )
    parser.add_argument(
