...
SELECT toString(suspect_commit) AS commit_sha, signal_key,
       argMax(tuple(verdict, confidence), timestamp) AS vc
...
def fetch_regressions(
    ...
    for commit_sha, signal_key in rows:
        if not is_test_signal(signal_key):
            continue
        regressions.setdefault(commit_sha, set()).add(signal_key)
    return regressions
...
def fetch_flaky_for_day(
    ...
    for _workflow, signal_key, commit_sha in rows:
        if not is_test_signal(signal_key):
            continue
        found.add((commit_sha, signal_key))
    return found
...
) -> Dict[Tuple[str, str], Tuple[str, Optional[float]]]:
    verdicts: Dict[Tuple[str, str], Tuple[str, Optional[float]]] = {}
    for i in range(0, len(shas), PUSH_CHUNK_SIZE):
        chunk = shas[i : i + PUSH_CHUNK_SIZE]
        rows = run_query(client, ADVISOR_SQL, {"repo": repo, "shas": chunk})
        for commit_sha, signal_key, vc in rows:
            verdict, confidence = vc
            conf = float(confidence) if confidence is not None else None
            verdicts[(commit_sha, signal_key)] = (verdict, conf)
    return verdicts
