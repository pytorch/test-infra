...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Path to the root of the local pytorch repository. If omitted, uses the main branch from github pytorch/pytorch",
    )
...
def compare_runner_types(
    ...
    if len(keys_not_in_both) > 0:
        if len(keys_not_in_both) == 1 and "ami" in keys_not_in_both and ignore_ami:
            ...
            print(
                f"Runner type {runner1_type} and {runner2_type} do not contain matching configs: {keys_not_in_both} is missing"
            )
            are_same = False
    ...
    for key in runner1_config:
        ...
        if runner1_config[key] != runner2_config[key]:
            print(
                f"Runner type {runner1_type} and {runner2_type} have different configurations for key {key}: {runner1_config[key]} vs {runner2_config[key]}"
            )
            are_same = False
...
def validate_internal(runner_types: dict) -> bool:
    errors_found = False
...
def validate_across_configs(
    source_config: dict, dest_config: dict, expected_prefix: str
) -> bool:
    errors_found = False

    # Every entry in the source_config should be in the dest_config with the same settings, except that the runner_type should have the expected_prefix
...
"""
    ...
    # repo_scale_config = copy.deepcopy(source_config)

    # # Add our custom prefix to all runner types
    # for runner_type in source_config[RUNNER_TYPE_CONFIG_KEY]:
    #     repo_scale_config[RUNNER_TYPE_CONFIG_KEY][f'{expected_prefix}{runner_type}'] = repo_scale_config[RUNNER_TYPE_CONFIG_KEY].pop(runner_type)

    # with open(dest_config_file, 'w') as f:
    #     f.write('# This file is generated by .github/scripts/validate_scale_config.py in test-infra\n')
    #     yaml.dump(repo_scale_config, f)


def load_scale_config(scale_config_path: str) -> dict:
    # Verify file exists
...
def main():
    ...
    if not validation_success:
        ...
        print()
        print(
            "Please run `python .github/scripts/validate_scale_config.py --test-infra-repo-root [path] --pytorch-repo-root [path]` locally to validate the scale-config.yml file and generate the updated pytorch scale config files"
        )
        print(
            "If this passes locally and you already have a PR open to update the pytorch scale config files, you can ignore this failure in CI"
        )
        exit(1)
