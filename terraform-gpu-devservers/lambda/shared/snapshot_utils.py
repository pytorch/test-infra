...
def get_latest_snapshot(user_id, volume_id=None, include_pending=False):
    ...
    try:
        ...
        logger.info(f"Found latest snapshot {latest_snapshot['SnapshotId']} ({latest_snapshot['State']}) for user {user_id}")
...
def cleanup_all_user_snapshots():
    ...
    try:
        ...
        logger.info(f"Scheduled snapshot cleanup completed: cleaned up {total_deleted} snapshots for {len(users_snapshots)} users")
