...
def process_wheel(whl_file, output_dir=None):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with InWheelCtx(whl_file) as ctx:
            ...
            dist_info_dirs = [d for d in os.listdir(ctx.path)
            ...
            for root, dirs, files in os.walk(os.path.join(ctx.path, dist_info_dir)):
                for file in files:
                    ...
                    try:
                        ...
                        updated_content = content.replace(version_with_suffix, version_no_suffix)
            ...
            new_dist_info_dir = dist_info_dir.replace(version_with_suffix, version_no_suffix)
        ...
        new_wheel_filename = wheel_filename.replace(version_with_suffix_escaped, version_no_suffix)
...
def main():
    parser = argparse.ArgumentParser(description='Prepare wheel files for PyPI by removing version suffixes.')
    parser.add_argument('wheel_files', nargs='+', help='Path to wheel files')
    parser.add_argument('--output-dir', default=None, help='Directory to output processed wheels (default: current directory)')
    ...
    try:
        ...
        print("Error: auditwheel package is not installed. Install it with 'pip install auditwheel'.")
        sys.exit(1)
